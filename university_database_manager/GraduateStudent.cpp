...
GraduateStudent::GraduateStudent() : Student() {
	...
	for (int i = 0; i < m_maxNumGraduateCourses; i++) {
		m_studentRecord[i] = nullptr;
	}
}

GraduateStudent::GraduateStudent(string studentName, int studentID) :
...
bool GraduateStudent::checkingCourse(string courseNumber) {
...
bool GraduateStudent::registeringCourse(Grade courseGrade) {
	...
	for (int j = 0; j < m_maxNumGraduateCourses; j++) {
...
bool GraduateStudent::droppingCourse(string courseNumber) {
